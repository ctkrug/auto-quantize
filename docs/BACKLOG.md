...
- [x] **1.1 [WOW] `recommend` prints one quant + one reason, end to end**
  - Running `snug recommend <hf-repo>` against a real public GGUF
...
- [x] **1.2 Real Linux hardware probing**
  - On a Linux host with an NVIDIA GPU, `snug probe` reports a
...
- [x] **3.1 `--json` machine-readable output**
  - `snug recommend <repo> --json` emits a single JSON object to
...
- [x] **3.4 Polished `--help` and error output**
  - `snug --help` and `snug recommend --help` document
